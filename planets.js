...
// In this exercise, we use the filter() method to find which planets could be habitable.
...
const planets = [
  ...
  const habitablePlanets = planets.filter(function (planet) {
	const rightTemperature =
	  planet.temperature >= 253 && planet.temperature <= 323;
	const rightDistance =
	  planet.distance >= 0.75 && planet.distance <= 1.5;
  
	return rightTemperature && rightDistance;
  });
  
  
  const habitablePlanetNames = habitablePlanets.map(function (planet) {
	return planet.name;
  });
